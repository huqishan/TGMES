...
{
    public class CAN : ICommunicationProtocol
    {
        #region Propertys
        public ConnectStatus IsConnected => throw new NotImplementedException();
        ...
        public CAN(CommuniactionConfigModel config)
        {
            ...
            {
                WriteLog(new LogMessageModel() { Message = "打开设备失败,请检查设备类型和设备索引号是否正确", Type = Abstractions.Enum.LogType.ERROR });
            }
            IntPtr property = DBC_Method.GetIProperty(m_dev_handle);
            if ((int)property == 0)
            {
                WriteLog(new LogMessageModel() { Message = "获取属性失败", Type = Abstractions.Enum.LogType.ERROR });
            }
            ...
            {
                WriteLog(new LogMessageModel() { Message = "启动CAN失败!", Type = Abstractions.Enum.LogType.ERROR });
            }
        }
        public bool Close()
        {
